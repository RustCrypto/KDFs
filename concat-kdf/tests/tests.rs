...
fn test_errors() {
    ...
    assert_eq!(
        ...
        Err(concat_kdf::Error::ZeroOutput)
    );
}

#[test]
#[ignore = "The test requires huge memory allocation."]
fn test_counter_overflow() {
    let mut huge_key = vec![0; Sha224::output_size() * u32::MAX as usize];
    assert_eq!(
        concat_kdf::derive_key_into::<Sha224>(&[0u8; 42], &[], &mut huge_key),
